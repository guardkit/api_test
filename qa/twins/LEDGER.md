...
| 2026-08-14 | users-delete-by-email | src/users/crud.py delete_user (and update_user, same missing-commit shape, untested by these twins) | adjudication PENDING — Rich's word needed on landing the crud commit fix on ddd-demo (and whether update_user gets the same fix + a twin) | see PILOT-REPORT + the two envelope runs above |
